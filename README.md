...
- Canva-style editing, all built in (no paid services): undo/redo (toolbar buttons or Ctrl/⌘+Z, Ctrl/⌘+Shift+Z / Ctrl+Y) where a whole drag or burst of typing is one step and undoing a delete restores the item; snapping to the photo's center lines and other markers with orange guide lines (hold Alt/Option to drag freely); keyboard shortcuts — arrow keys nudge (Shift = 10px), Ctrl/⌘+D duplicates, Delete removes, Esc deselects
- "Export IG Story" renders the look as a 1080×1920 JPEG — photo, markers, arrows, text, the site wordmark, and the look's link — and opens the phone's share sheet (straight to Instagram) or downloads it on desktop (`src/lib/story-export.ts`). The photo comes through Next's same-origin image optimizer so a cross-origin (Blob) photo can't taint the canvas
- Editor autosave keeps one debounce timer per item — a single shared timer used to drop item A's pending save whenever item B changed within the same second
- Drawing arrows and dropping text use pointer events, so they work with a finger on phones, not just a mouse
...
- Pick an aspect ratio when uploading (Original, 4:5, 1:1, 3:4, 2:3, 9:16, 16:9 — 4:5 by default, 1:1 for the profile photo), then drag the frame and zoom to choose what to keep; the crop happens in the browser before upload (`src/components/editor/image-cropper.tsx`)
- Existing looks can be re-cropped from the editor ("Crop / ratio"): the server crops the stored photo with `sharp` (EXIF-orientation aware) and moves every hotspot, arrow, and text label into the new frame so they stay on the same spot; anything that falls outside is pinned to the edge and the admin is told to check it (`/api/shoppable-images/[id]/crop`, `reframeShoppableImage` in `src/lib/data.ts`)
...
- Category management (`/admin/categories`): create, rename, reorder (used as the public sort order), archive/restore, delete, and pick an icon from a curated lucide-react set plus hand-drawn garment icons lucide lacks — pants, hijab, dress/gamis, skirt (`src/components/admin/category-icons.tsx`, `clothing-icons.tsx`) — also selectable when uploading a new look
...
- English/Indonesian: every public page is translated (`src/lib/i18n/dictionaries.ts`), defaults to English, and remembers the visitor's EN/ID choice in a cookie; `<html lang>` and `og:locale` follow it. The admin is bilingual too (`src/lib/i18n/admin-dictionaries.ts`, EN/ID toggle in the admin sidebar), kept in its own dictionary so storefront bundles don't ship admin copy
...
- Once-per-session intro splash (`src/components/storefront/splash-screen.tsx`): the site name (from `/admin/settings`) drawn as an outline that fills from the bottom like rising water. The fill follows real readiness (window load + fonts) rather than a fixed timer — at least 2.5s so the fill is actually visible on fast connections, at most 7s so a slow connection never traps visitors. A session cookie lets the server skip it on later page views; never shown in `/admin`; respects reduced motion. The wordmark font is Lazy Dog by Paul Neave (freeware), self-hosted from `public/fonts/Lazydog.ttf`
- Admin is no longer linked from the public navigation — go to `/admin` directly
- Replaced, re-cropped, and deleted photos are removed from storage (Vercel Blob or `public/uploads/`) once no look or profile photo still uses them — duplicated looks share their original's file, so it's only deleted when the last one goes
- Analytics events carry a BSON `ts` date and expire after `ANALYTICS_RETENTION_DAYS` (default 365) via a TTL index that `npm run db:indexes` creates, backfilling older events
- Click rate limiting uses Upstash Redis (REST, no extra dependency) when configured, falling back to in-memory
- Unpublished or deleted looks return a real 404 status (the home skeleton lives in a `(home)` route group so its streaming boundary doesn't wrap `/p/[slug]`)
- 33 unit tests covering the data layer (pagination, category matching, homepage selection, duplicate, delete cascade and shared-photo cleanup, crop remapping, search), crop math, rate limiting, and EN/ID dictionary parity
