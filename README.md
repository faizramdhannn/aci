...
- Once-per-session intro splash (`src/components/storefront/splash-screen.tsx`): the site name (from `/admin/settings`) drawn as an outline that fills from the bottom like rising water. The fill follows real readiness (window load + fonts) rather than a fixed timer — at least 0.7s so it doesn't just flash, at most 6s so a slow connection never traps visitors. A session cookie lets the server skip it on later page views; never shown in `/admin`; respects reduced motion. The wordmark font is Lazydog, self-hosted from `public/fonts/Lazydog.woff2` (or `.otf`/`.ttf`) — until that file is added it falls back to Playfair Display
