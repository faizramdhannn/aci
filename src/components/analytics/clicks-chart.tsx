...
export function ClicksChart({ data }: { data: { date: string; clicks: number; views: number }[] }) {
  if (data.length === 0) {
    return <p className="text-sm text-brown-soft">No activity yet — clicks and views will show up here.</p>;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <LineChart data={data} margin={{ left: -20, right: 10 }}>
        ...
        />
        <Line type="monotone" dataKey="views" stroke="#8A6A54" strokeWidth={2} dot={false} />
        <Line type="monotone" dataKey="clicks" stroke="#E5781E" strokeWidth={2} dot={false} />
      </LineChart>
    </ResponsiveContainer>
  );
}
