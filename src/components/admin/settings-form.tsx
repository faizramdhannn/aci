...
export function SettingsForm({ initial }: { initial: SiteSettings }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      toast(data.error ?? t.settings.failed, "error");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <label className="block text-sm">
        <span className="mb-1 block text-brown-soft">{t.settings.siteName}</span>
        <input required {...field("siteName")} className={inputClass} />
      </label>
      <label className="block text-sm">
        <span className="mb-1 block text-brown-soft">{t.settings.yourName}</span>
        <input {...field("creatorName")} placeholder={t.settings.namePlaceholder} className={inputClass} />
      </label>
      <label className="block text-sm">
        <span className="mb-1 block text-brown-soft">{t.settings.tagline}</span>
        <input {...field("tagline")} placeholder={t.settings.taglinePlaceholder} className={inputClass} />
      </label>
      <label className="block text-sm">
        <span className="mb-1 block text-brown-soft">{t.settings.about}</span>
        <textarea {...field("about")} rows={3} className={inputClass} />
      </label>
      <div className="text-sm">
        <span className="mb-1 block text-brown-soft">{t.settings.photo}</span>
      ...
      <label className="block text-sm">
        <span className="mb-1 block text-brown-soft">{t.settings.instagram}</span>
        <input type="url" {...field("instagramUrl")} placeholder="https://instagram.com/…" className={inputClass} />
      </label>
      <label className="block text-sm">
        <span className="mb-1 block text-brown-soft">{t.settings.tiktok}</span>
        <input type="url" {...field("tiktokUrl")} placeholder="https://tiktok.com/@…" className={inputClass} />
      </label>
      <label className="block text-sm">
        <span className="mb-1 block text-brown-soft">{t.settings.email}</span>
