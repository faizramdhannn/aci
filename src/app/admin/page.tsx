...
export async function generateMetadata(): Promise<Metadata> {
  return { title: (await getAdminDictionary()).overview.title };
}
export const dynamic = "force-dynamic";

export default async function AdminOverviewPage() {
  const [images, summary, t] = await Promise.all([listShoppableImages(), getAnalyticsSummary(), getAdminDictionary()]);

  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-brown">{t.overview.title}</h1>
        <Link
      ...
      <div className="mb-10 grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard label={t.overview.views} value={summary.totalViews} />
        <StatCard label={t.overview.clicks} value={summary.totalClicks} />
        <StatCard label={t.overview.ctr} value={`${(summary.ctr * 100).toFixed(1)}%`} />
        <StatCard label={t.overview.sessions} value={summary.uniqueSessions} />
      </div>

      <h2 className="mb-4 text-lg font-semibold text-brown">{t.overview.looks}</h2>
      {images.length === 0 ? (
            ...
            >
              ...
              >
                ...
                <span className="absolute right-2 top-2 rounded-full bg-brown/80 px-2 py-0.5 text-[10px] uppercase tracking-wide text-cream">
                  {image.status === "published" ? t.common.statusPublished : t.common.statusDraft}
                </span>
              </div>
